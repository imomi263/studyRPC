...
public class ZookeeperDiscoveryService implements DiscoveryService {
    ...
    public ServiceInfo discovery(String serviceName) throws Exception {
        ...
        return CollectionUtils.isEmpty(serviceInstances) ? null : loadBalance.chooseOne(serviceInstances.stream().map(ServiceInstance::getPayload).collect(Collectors.toList()));
